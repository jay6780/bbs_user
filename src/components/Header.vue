            ...
            <div class="box">
              ...
              <div class="nav">
                <ul class="clearfix">
                  <el-skeleton
                    ...
                    style="width: 300px; position: relative; top: -3px"
                  >
                    <template #template>
                      <div
                        style="
                      ...
                      >
                        <el-skeleton-item
                          variant="text"
                          style="width: 30%; height: 30px; margin-right: 16px"
                        />
                        <el-skeleton-item
                          variant="text"
                  ...
                  <li v-for="column in state.columnList">
                    <a
                      :href="column.url"
                      v-if="column.linkMode == 2"
                      target="_blank"
                      >{{ column.name }}</a
                    >
                    <router-link tag="a" :to="'/' + column.url" v-else>{{
                      column.name
                    }}</router-link>
                  </li>
                </ul>
              ...
              <div class="menu">
                <ul class="clearfix">
                  ...
                  <li
                    v-if="
                      state.languageSwitchingOptions != null &&
                  ...
                  >
                    <el-dropdown>
                      ...
                      <template #dropdown>
                        <el-dropdown-menu>
                          <el-dropdown-item
                            @click="selectLanguage(languageSwitching)"
                            v-for="languageSwitching in state.languageSwitchingOptions"
                            ><i
                              class="language-placeholder-icon"
                              v-if="state.isSelectedLanguage"
                              ><icon
                                v-if="languageSwitching.selected"
                                name="themify-check"
                                size="15px"
                            /></i>
                            {{ languageSwitching.name }}
                          </el-dropdown-item>
                        </el-dropdown-menu>
                      </template>
                    </el-dropdown>
                  ...
                  <li>
                    <div
                      v-if="
                        $store.state.systemUser == null ||
                        Object.keys($store.state.systemUser).length == 0
                      "
                    >
                      <!-- 注册 -->
                      <li>
                        <router-link tag="a" to="/register" class="a-button">{{
                          t("header.10")
                        }}</router-link>
                      </li>
                      <!-- 登录 -->
                      <li>
                        <a @click="loginLink" class="a-button">{{
                          t("header.20")
                        }}</a>
                      </li>
                    </div>
                  ...
                  >
                    <el-dropdown>
                      <div class="message">
                        <el-badge
                          class="message-el-badge"
                          :max="99"
                          :value="state.unreadMessageCount"
                          :hidden="state.unreadMessageCount == 0"
                        >
                          <el-icon size="26px">
                            <BellFilled />
                          </el-icon>
                        ...
                        <el-dropdown-menu>
                          ...
                          <el-dropdown-item
                            ...
                            ><i class="icomoon-icon"
                              ><icon name="email" size="15px"
                            /></i>
                            {{ t("header.40") }}
                            <el-badge
                              :max="99"
                              :value="state.unreadMessage.privateMessageCount"
                              :hidden="
                                state.unreadMessage.privateMessageCount == 0
                              "
                              class="custom-el-badge"
                          /></el-dropdown-item>
                          <!-- 提醒 -->
                          <el-dropdown-item
                            @click="
                              router.push({ path: '/user/control/remindList' })
                            "
                            ><i class="icomoon-icon"
                            ...
                            <el-badge
                              :max="99"
                              :value="state.unreadMessage.remindCount"
                              :hidden="state.unreadMessage.remindCount == 0"
                              class="custom-el-badge"
                          /></el-dropdown-item>
                          <!-- 系统通知 -->
                          <el-dropdown-item
                            ...
                            "
                            ><i class="icomoon-icon"
                              ><icon name="announcement" size="15px"
                            /></i>
                            {{ t("header.60") }}
                            <el-badge
                              :max="99"
                              :value="state.unreadMessage.systemNotifyCount"
                              :hidden="
                                state.unreadMessage.systemNotifyCount == 0
                              "
                              class="custom-el-badge"
                          /></el-dropdown-item>
                        </el-dropdown-menu>
                    ...
                    ><router-link
                      tag="a"
                      to="/user/control/privateMessageList"
                      class="a-pic"
                    ></router-link>
                  </li>
                  <li>
                    ...
                    >
                      <el-dropdown>
                        <router-link tag="a" to="/user/control/home">
                          <img
                            v-if="$store.state.systemUser.avatarName != null"
                            :src="
                              $store.state.systemUser.avatarPath +
                              '100x100/' +
                              $store.state.systemUser.avatarName
                            "
                          />
                          ...
                          >
                            <span class="circle"
                              ><Icon name="user2" size="28px" class="icon"
                            /></span>
                          </span>
                          <el-icon size="15px" class="arrow"
                            ><CaretBottom
                          /></el-icon>
                        </router-link>
    ...
    if (node.getAttribute("class")?.indexOf("el-affix--fixed") == 0) {
      if (
        (oldPosition < value.scrollTop &&
          value.fixed &&
          value.scrollTop > 200) ||
        (props.shrink && value.scrollTop > 200)
      ) {
        //如果向下滚动并且在浮动状态并且滚动200像素
        //if(oldPosition < value.scrollTop && value.fixed){//如果向下滚动并且在浮动状态
        node.setAttribute(
          "class",
          "el-affix--fixed custom-el-affix--fixed--no",
        );
      ...
      .then((response: AxiosResponse) => {
        if (response) {
          ...
          state.unreadMessageCount =
            parseInt(result.privateMessageCount) +
            parseInt(result.remindCount) +
            parseInt(result.systemNotifyCount);
          state.unreadMessage.privateMessageCount = parseInt(
            result.privateMessageCount,
          );
          state.unreadMessage.remindCount = parseInt(result.remindCount);
          state.unreadMessage.systemNotifyCount = parseInt(
            result.systemNotifyCount,
          );
...
onMounted(() => {
  ...
  let keyword: string =
    router.currentRoute.value.query.keyword != undefined
