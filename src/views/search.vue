            ...
            <div class="searchModule">
                <div class="topic-box">
                    <div class="topicList">
                        ...
                        <el-result v-if="state.loading == false && state.isFinished && (state.searchResultPage.records == undefined || state.searchResultPage.records.length ===0)" icon="info" :title="t('search.90')" :sub-title="t('search.100')"></el-result>
                    </div>
                </div>      
            </div>
        </div>
                ...
                if(result){
                    if(result.records != null && result.records.length >0){
                        for(let i:number=0; i<result.records.length; i++){
                            let searchResult = result.records[i];
                    ...
                    state.searchResultPage = result;
                    state.totalrecord = parseInt(result.totalrecord);//服务器返回的long类型已转为String类型
                    state.currentpage = result.currentpage;
                    state.totalpage = parseInt(result.totalpage);//服务器返回的long类型已转为String类型
                    state.maxresult = result.maxresult;
