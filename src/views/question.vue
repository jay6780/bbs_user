            ...
            <el-affix position="top" :offset="140">
                <div class="leftFloatPanel">
                    <el-badge :value="(Long.fromString(state.likeCount).gt(99999) ? '99999+' : state.likeCount)" :class="state.alreadyLiked == true ? 'active':''">
                        <!-- content="点赞" -->
                        <el-tooltip effect="tag-customized" :content="t('question.670')" placement="right-end" transition=" " :hide-after="10">
                    ...
                    <el-badge :value="(Long.fromString(state.favoriteCount).gt(99999) ? '99999+' : state.favoriteCount)" :class="state.isFavorited == true ? 'active':''">
                        <!-- content="收藏" -->
                        <el-tooltip effect="tag-customized" :content="t('question.120')" placement="right-end" transition=" " :hide-after="10">
                            <div class="circle" @click="()=>{state.isFavorited == true ? deleteFavorite(state.question.id):addFavorite(state.question.id)}">
                    ...
                    <el-badge :value="state.totalrecord" :max="99999" class="item">
                        <!-- content="答案" -->
                        <el-tooltip effect="tag-customized" :content="t('question.660')" placement="right-end" transition=" " :hide-after="10">
                    ...
                    <div class="question-wrap">
                        ...
                        <div class="questionBox">
                            ...
                            <div class="favorite-formModule" v-if="state.question.userName != null && state.question.userName != ''">
                                <!--content="点赞"-->
                                <el-tooltip effect="tag-customized" :content="t('question.670')" placement="top" transition=" " :hide-after="10">
                                ...
                                <!--content="收藏"-->
                                <el-tooltip effect="tag-customized" :content="t('question.120')" placement="top" transition=" " :hide-after="10">
                            ...
                            <div :class="[answer.adoption == true ? 'answer active' : 'answer',router.currentRoute.value.query.reportModule !=undefined && parseInt(router.currentRoute.value.query.reportModule as string) == 50 && answer.id == router.currentRoute.value.query.answerId ? 'reportMark' : '']" v-for="(answer,index) in state.answerList" :key="answer.id"  :ref="'answer_'+answer.id" v-lazy-tag="()=>{onQueryLike(answer.id,50)}">
                                ...
                                <div class="replyList" :class="'affix-container_reply_'+answer.id" v-if="answer.answerReplyList != null && answer.answerReplyList.length >0">
                                    <ul class="timeline box">
                                        <!-- 延迟加载点赞 -->
                                        <li class="timeline-item replyItem-container" v-for="(reply,index2) in answer.answerReplyList" :key="reply.id" :replyId="reply.id" :ref="el => handleReplyNode(el, reply.id)"  >
                                            <div v-lazy-tag="()=>{onQueryLike(reply.id,60)}" v-if="state.replyExpandOrShrink.get(answer.id) || (state.replyExpandOrShrink.get(answer.id) == false && index2 <state.viewNumber)">
                                            ...
                                            <div class="replyItem">
                                                <div :class="router.currentRoute.value.query.reportModule !=undefined && parseInt(router.currentRoute.value.query.reportModule as string) == 60 && reply.id == router.currentRoute.value.query.replyId ? 'reply-reportMark' : ''">
                                                    ...
                                                    <!-- 修改回复 -->
                                                    <div class="editAnswerReply-formModule" v-if="state.editReplyFormView.get(reply.id)">
                                                    ...
                                                    <!-- 回复对方 -->
                                                    <div class="addAnswerReplyFriend-formModule" v-if="state.addReplyFriendFormView.get(reply.id)">
                                ...
                                <!-- 添加回复 -->
                                <div class="answerReply-formModule" v-if="state.addReplyFormView.get(answer.id)">
                                    <div class="addReply-wrap">
                                        ...
                                        <el-form label-position="right" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                            <el-form-item :error="error.replyContent.get('addReply-'+answer.id)">
                                                <div style="width: 100%;">
                                                    <textarea :ref="el => handleAddReplyNode(el, answer.id)" :editorId="'addReply_'+answer.id" :answerId="answer.id" style="width:100%;height:230px;visibility:hidden;"></textarea>
                                                </div>
                                            </el-form-item>
                                ...
                                <!-- 修改答案表单 -->
                                <div class="editAnswer-formModule" v-if="state.editAnswerFormView.get(answer.id)">
                                        ...
                                        <el-form label-position="right" :ref="'formEditAnswerRef-'+answer.id" size="large" :model="state" class="iconForm-container" @submit.native.prevent>
                                            <el-form-item :error="error.content.get('editAnswer-'+answer.id)">
                                                ...
                                                <div v-if="!state.isEditAnswerMarkdownMap.get(answer.id)" style="width: 100%;">
                                                    <textarea :ref="el => handleEditAnswerNode(el, answer.id)" :editorId="'editAnswer_'+answer.id" :answerId="answer.id" style="width:100%;height:230px;visibility:hidden;"></textarea>
                                                </div>
                                                <div v-if="state.isEditAnswerMarkdownMap.get(answer.id)" style="width: 100%;">
                                                    <!--placeholder="请输入内容..."-->
                    ...
                    <div class="affix-container">
                        <el-affix target=".affix-container" :offset="state.affix_offset" z-index="1" @scroll="onRightScroll">
                            <div class="right-container" ref="right_ref">
                                ...
                                <!--  相似问题 集合 -->
                                <div class="similarQuestionModule clearfix" v-if="state.similarQuestionList != null && state.similarQuestionList.length >0">
                                    <div class="similarQuestion">
                                        <!-- 相关问题-->
                                        <div class="title">{{t('question.380')}}</div>
                                        <ul class="questionList">
                                            <li class="question-title " v-for="similarQuestion in state.similarQuestionList">
                                                <router-link tag="a" :to="{path: '/question', query:{ questionId : similarQuestion.id}}">
...
<script lang="ts" setup>
    ...
    const state = reactive({
		...
		editAnswerElementNodes:new Map<string,HTMLTextAreaElement | null>(),//修改答案项Element节点集合
        ...
        editReplyElementNodes:new Map<string,HTMLTextAreaElement | null>(),//修改回复Element节点集合
        ...
        similarQuestionList:[] as Array<Question>, //相似问题集合
        ...
        .then((data: PageView<Answer>) => {
            ...
            if(data.records != null && data.records.length >0){
                for(let i:number=0; i<data.records.length; i++){
                    ...
                    if(!state.replyExpandOrShrink.get(answer.id)){//如果之前是收缩的或不存在的
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(result.success){
                    ...
                    hasUserFavoritedQuestion(questionId);
    ...
    //用户是否已经收藏问题
    const hasUserFavoritedQuestion = (questionId:string) => {
        if(store.state.systemUser == null || Object.keys(store.state.systemUser).length==0){
            return;
        }
        proxy?.$axios({
            url: '/hasUserFavoritedQuestion',
            method: 'get',
            params:  { 
                questionId: questionId,
            },
        ...
        .then((response: AxiosResponse) => {
            if(response){
                ...
                if(module == 40){
                    ...
                    state.alreadyLikedAnswer.set(itemId,data.isLiked)
                }else if(module == 60){
                    state.alreadyLikedAnswerReply.set(itemId,data.isLiked)
                }
    ...
    const appendQuestionUI = (questionId: string) => {
        ...
        proxy?.$axios({
            url: '/user/getAppendQuestionForm',
            method: 'get',
            params:  {
                questionId: questionId
            },
    ...
    const adoptionAnswer = (answerId:string) => {
        ...
        .then(() => {
            ...
            proxy?.$axios({
                url: '/user/control/answer/adoptionAnswer',
                method: 'post',
                data: formData
            })
            .then((response: AxiosResponse) => {
    ...
    const editReplyUI = (reply:AnswerReply) => {
        ...
        proxy?.$axios({
            url: '/user/getEditAnswerReplyForm',
            method: 'get',
            params:  {
                replyId: reply.id
            },
    ...
    //查询相似问题
    const querySimilarQuestions = (questionId:string) => {
        proxy?.$axios({
            url: '/querySimilarQuestions',
            method: 'get',
            params:  {
                questionId:questionId
            },
        ...
        .then((data: Array<Question>) => {
            state.similarQuestionList.length =0;

            if(data){
                state.similarQuestionList = data;
            }
        }).catch((error: any) =>{
            console.log(error);
        });
    ...
    const clearEditor = () => {
        ...
        state.editAnswerMarkdownContent = {} as any;//修改答案markdown编辑内容 key:答案Id value:评论内容
    ...
    let watch_systemUser = watch(() => store.state.systemUser, (val, old) => {
        if(store.state.systemUser != null && Object.keys(store.state.systemUser).length >0){
            if(state.questionId && !state.alreadyLiked){
                hasUserLikedTopic(state.questionId,40);
            }
            if(state.questionId && !state.isFavorited){
                hasUserFavoritedQuestion(state.questionId);
            }
            ...
            .questionBox{
                ...
                .reward{
                    ...
                    .info{
                        ...
                        .point{
                            color: #cca558;margin-left: 5px; margin-right: 5px;font-weight: 700;font-size: 18px;position: relative;top: 1px;
                        }
                    }
                }
                //举报标记
