        ...
        if(store.state.systemUser != null && Object.keys(store.state.systemUser).length>0){//如果在登录状态
            ...
            .then((response: AxiosResponse) => {
                if(response){
                    ...
                    state.unreadMessageCount = parseInt(result.privateMessageCount) + parseInt(result.remindCount) + parseInt(result.systemNotifyCount);
                    state.unreadMessage.privateMessageCount = parseInt(result.privateMessageCount);
                    state.unreadMessage.remindCount = parseInt(result.remindCount);
                    state.unreadMessage.systemNotifyCount = parseInt(result.systemNotifyCount);
